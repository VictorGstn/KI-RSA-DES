...
void send(string message){
    ...
    fprintf(msg, "%s\n", message.c_str());
    fclose(msg);
    system("nc 10.16.1.2 3333 -q 0 < message.txt");
}
...
int main(){  
	...
	string id,N1;
	split(msg_rcv, &id,&N1);
    int N1_int = modpow(stoi(N1),d,n);
    int id_int = modpow(stoi(id),d,n);
    cout<<"id and N1 received:" <<msg_rcv<<endl;
    cout<<"N1 decrypt: "<<N1_int<<endl;
    cout<<"id decrypt: "<<id_int<<endl;
	...
	cout<<"N1: "<<N1_int<<endl;
	cout<<"N2: "<<N2<<endl;
	msg = to_string(modpow(N1_int,e1,n1));
	msg += ";";
	msg += to_string(modpow(N2,e1,n1));
	send(msg);
    cout<<"N1 and N2 send:" <<msg<<endl;

    //receive N2
    msg_rcv = receive();
	int m2_N2_int = modpow(stoi(msg_rcv),d,n);
    if(m2_N2_int==N2){
		cout<<"n2 matched"<<endl;
	} else{
		cout<<"n2 doesn't match"<<endl;
		//exit(1);
	}
    cout<<"N2 receive: "<<endl;
    ...
    while(i<8){
		msg_rcv = receive();
        sess_key += to_string(modpow(stoi(msg_rcv),d,n)); 
        cout<<"key: "<<sess_key<<endl;
        i++;
	}

	cout<<"session key: "<<sess_key<<endl;
	struct arrWrap keys;
	sess_key = convertHexToBinary(sess_key);
	keys=generate_keys(sess_key); 
	int num = sizeof(keys.arr)/sizeof(keys.arr[0]);
	reverse(keys.arr, keys.arr+num);  

	msg_rcv = receive();
	string dt = DES(convertHexToBinary(string_to_hex(msg_rcv)), keys);
